...
In this section we provide readers with some details about the state of the art for symbolic knowledge extraction. % (\xss{extraction}) and the \psyke{} platform (\xss{psyke}).
...
In this section we present a general model for hypercube-based extractors (\xss{unified-model}).
%
...
\subsubsection{Output rule set creation}\labelssec{rule}
...
In the following a comparison between two hypercube-based SKE algorithms %cited in \xs{experiments} 
...
%
Output rules are lists of logic rules in both cases, following the convention described in \xss{rule}.
%
