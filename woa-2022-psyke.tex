...
%%
\title{Formal Generalisation of Symbolic Knowledge Extraction Algorithms: Shattering Task Constraints}
...
\begin{abstract}
Symbolic knowledge-extraction (SKE) algorithms proposed by the explainable artificial intelligence community to obtain human-intelligible explanations for opaque predictors' behaviour are currently studied and developed with growing interest.
%
Choosing the most adequate extraction procedure amongst the existing in the literature becomes more and more challenging with the design of new methods.
%
However, the majority of the proposed algorithms come along with restricting application constraints, for instance regarding the underlying black-box structure, the kind of the task at hand, or the nature of input features.
%
The former limitation is overcome by \emph{pedagogical} extractors, applicable to any kind of black box regardless of their internal structure.
%
The latter may be bypassed thanks to feature manipulation, e.g., discretisation of continuous features or one-hot encoding of real-valued features.
%
Unfortunately, no workarounds exist to adapt extractors explicitly designed for regression to classification tasks, especially if they perform generalisations via averaging operations and threshold-based similarity comparisons.

Accordingly, in this paper we propose an adaptation of SKE techniques aimed at extending the application of these algorithms without being subject to task limitations.
%
...
\begin{keywords}
	explainable AI \sep
  	knowledge extraction \sep
	interpretable prediction \sep
  	PSyKE
\end{keywords}
